...
create table drug_catalogue (
    ...
    Drug_measurement varchar(30) not null,
...
/* addDrug() */
INSERT INTO drug_catalogue (`Drug_id`, `Drug_name`, `Drug_type`, `Drug_measurement`, `Drug_quantity`, `Drug_prescription`, `Drug_Description`, `Timestamp`) 
	VALUES(10001,"Panadol","branded","20mg",20,"1X2","Treat headaches",now()),
    (10002,"Rob","generic","100g",20,"1X3","Relieves pain",now()),
    (10003,"Sonadol","generic","20mg",20,"1X2","Treat headaches",now());
...
insert into staff_details (`Staff_id`,`Password`,`ID`,`First_name`,`Last_name`,`Other_name`,`Gender`,`DOB`,`Phone`,`Email`,`Title`,`Salary`,`Perks`,`Employed_date`) 
	values ("1003","5432",56724321,"Musau","Njoki","Wango","female","1990-03-21",0715352899,"njoki@yahoo,com","staff",100000,20000,date(current_date()));          
...
/* confirmDrug() from drug_purchases */
-- select distinct drug_catalogue.`Drug_id`,`Drug_name`,`Drug_measurement`,`Drug_quantity`,`Sell_price`,`Drug_prescription`,`Drug_description` 
...
-- SELECT * FROM confirm_drug WHERE `Drug_name` = ? AND `Drug_measurement` = ? ;

/* confirmDrugCat() */
-- select distinct `Drug_id`, `Drug_name`, `Drug_type`, `Drug_measurement`, `Drug_quantity`
-- 	from drug_catalogue where `Drug_name` = "lonart" and `Drug_measurement` = "50mg";
    
create view confirm_drug_catalogue as 
	select distinct `Drug_id`, `Drug_name`, `Drug_type`, `Drug_measurement`, `Drug_quantity`
	from drug_catalogue;
-- SELECT * FROM confirm_drug_catalogue WHERE `Drug_name` = ? AND `Drug_measurement` = ? ;

/* fetch drugId */
-- SELECT `Drug_id` FROM drug_catalogue WHERE `Drug_name` = ? AND `Drug_measurement` = ? ;

/* monetaryDetails() */
/* companyDetails() */ 

/* terminateStaff() */
-- UPDATE staff_details SET `Status` = "Terminated", `Terminated_date` = current_date() WHERE `ID` = ? ;
-- select * from staff_details;

/* updateQuantity() */ 
UPDATE drug_catalogue SET `Drug_quantity` = `Drug_quantity` + ? WHERE `Drug_id` = ? ;
select * from drug_catalogue;

/* updateQty() */ 
-- UPDATE drug_catalogue SET `Drug_quantity` = `Drug_quantity` - ? WHERE `Drug_name` = ? AND `Drug_measurement` = ? ;
-- select * from drug_catalogue;

/* updateSalary() */ 
-- UPDATE staff_details SET `Salary` = ? WHERE `ID` = ? ;
-- select * from staff_details;

/* updatePerks() */
-- UPDATE staff_details SET `Perks` = ? WHERE `ID` = ? ;

/* updateEmail() */
-- UPDATE staff_details SET `Email` = ? WHERE `ID` = ? ;

/* updatePhone() */
-- UPDATE staff_details SET `Phone` = ? WHERE `ID` = ? ;

/* updatePassword() */
-- UPDATE staff_details SET `Password` = ? WHERE `ID` = ? ;

/* updateUsername() */
-- UPDATE staff_details SET `Staff_id` = ? WHERE `ID` = ? ;

/* fetchSales() */ 
-- select drug_sales.`Drug_id`,`Drug_name`,`Drug_measurement`,`Sale_quantity`,`Sell_price`,`Staff_id`, drug_sales.`Timestamp` 
...
/* fetchAvailableDrugs() */
-- SELECT `Drug_id`,`Drug_name`,`Drug_measurement`,`Drug_type`,`Drug_quantity` FROM drug_catalogue;

/* fetchExpiring() */ 
-- select drug_catalogue.`Drug_id`,`Drug_name`,`Drug_measurement`,`Drug_type`,`Drug_quantity`,`Buy_quantity`,`Drug_expiry` 
-- 	from drug_catalogue, drug_purchases
--     where drug_catalogue.`Drug_id` = drug_purchases.`Drug_id`;
...
-- SELECT * FROM fetch_expiring WHERE date(`Drug_expiry`) = date(current_date()); /* fetches today */ 
...
create view fetch_staff as 
	select `Staff_id`,`ID`,`First_name`,`Last_name`,`Other_name`,`Gender`,`DOB`,`Phone`,`Email`,`Title`,`Salary`,`Perks`,`Status`,`Employed_date`,`Terminated_date`
	from staff_details;
-- SELECT * FROM fetch_staff WHERE `ID` = ? ;
-- SELECT * FROM fetch_staff;
...
create view fetch_signIn as 
	select `Staff_id`,`Password`,`Title`,`Status`,`ID` from staff_details where `Status` not in("terminated");
-- SELECT * FROM fetch_signIn WHERE `Staff_id` = ? AND `Password` = ? ;

/* fetchUsername() */
create view fetch_username as
	select `Staff_id`, `ID`, `Status` from staff_details where `Status` not in("terminated");
